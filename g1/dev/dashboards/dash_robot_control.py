...
def get_depth_preview(robot: Any) -> _DepthPreviewReceiver:
...
def get_rgb_preview(robot: Any) -> _RgbPreviewReceiver:
    global RGB_PREVIEW
    with RGB_LOCK:
        if (
            RGB_PREVIEW is None
...
def get_robot() -> Any:
    """
    Return the Robot singleton, or None if not yet connected / failed.
    Only attempts to create the robot after the user clicks Connect.
    """
    global ROBOT_INSTANCE, ROBOT_INIT_ERR, ROBOT_IFACE, ROBOT_LIDAR_CLOUD_TOPIC
    with ROBOT_LOCK:
        if ROBOT_INSTANCE is not None:
            return ROBOT_INSTANCE
        if not ROBOT_CONNECT_REQUESTED:
            return None
...
def _robot_status() -> tuple[str, str]:
    """Return (label, badge_color) for the connection status badge."""
    with ROBOT_LOCK:
        if ROBOT_INSTANCE is not None:
...
def on_connection_action(
    ...
    global ROBOT_CONNECT_REQUESTED, ROBOT_INIT_ERR, ROBOT_INSTANCE
    ...
    if trigger == "btn-connect":
        with ROBOT_LOCK:
            ROBOT_CONNECT_REQUESTED = True
            # Allow retry after previous error
            ROBOT_INIT_ERR = None
            ROBOT_INSTANCE = None
        get_robot()  # attempt connection now

    label, color = _robot_status()
    detail = ""
    with ROBOT_LOCK:
        if ROBOT_INIT_ERR:
            detail = f"Error: {ROBOT_INIT_ERR}"
...
def on_control(
    ...
    robot = get_robot()
    if robot is None:
        label, _ = _robot_status()
        return f"Robot not connected ({label}). Click Connect first.", "warning", ""
...
def on_navigation(
    ...
    if robot is None:
        label, _ = _robot_status()
        return f"Robot not connected ({label}). Click Connect first."
...
def on_apply_iface(_n: int | None, iface_input: str | None) -> tuple[str, str]:
    global ROBOT_INSTANCE, ROBOT_INIT_ERR, ROBOT_IFACE, DEPTH_PREVIEW, RGB_PREVIEW, LIVOX_PREVIEW, ROBOT_CONNECT_REQUESTED
    ...
    with ROBOT_LOCK:
        ...
        ROBOT_INSTANCE = None
        ROBOT_INIT_ERR = None
        ROBOT_CONNECT_REQUESTED = False   # require explicit reconnect
    with DEPTH_LOCK:
        if DEPTH_PREVIEW is not None:
            DEPTH_PREVIEW.stop()
        DEPTH_PREVIEW = None
    ...
    return f"Iface updated to '{iface}'. Click Connect to reconnect.", iface
...
def update_rgb_feed(_tick: int, prev_src: str | None) -> tuple[str | None, str]:
    ...
    if robot is None:
        label, _ = _robot_status()
        return prev_src, f"Not connected ({label})."
...
def update_lidar(_tick: int) -> tuple[go.Figure, str, go.Figure, str]:
    ...
    if robot is None:
        label, _ = _robot_status()
        msg = f"Not connected ({label})."
        return (
            empty_lidar_figure("LiDAR — not connected"),
            msg,
            empty_imu_figure("IMU — not connected"),
            msg,
        )
