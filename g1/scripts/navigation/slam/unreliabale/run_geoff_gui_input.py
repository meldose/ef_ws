...
def _setup_logging() -> logging.Logger:  # noqa: D401 – small helper
    ...
    log_path = log_dir / "run_geoff_gui.log"
...
# ---------------------------------------------------------------------------
# Keyboard controller (terminal / curses / pynput)
# ---------------------------------------------------------------------------


class _BotProxy:
    """Proxy that mirrors bot methods while updating GUI velocity state."""

    def __init__(self, bot):
        self._bot = bot

    def Move(self, vx, vy, omega, continous_move=True):  # noqa: N802 - SDK API
        self._bot.Move(vx, vy, omega, continous_move=continous_move)
        with _state_lock:
...
def _run_keyboard_controller(stop: "threading.Event", iface: str, backend: str) -> None:
    if backend == "qt":
        return
    ...
    try:
        ...
        from keyboard_controller import (  # type: ignore
            drive_loop_curses,
            drive_loop_pynput,
        )

        bot = _BotProxy(hanger_boot_sequence(iface=iface))
        ...
        print("[run_geoff_gui] Keyboard controller disabled:", exc, file=sys.stderr)
...
class GeoffWindow(QtCore.QObject):  # type: ignore[misc]  # pylint: disable=too-few-public-methods
    def __init__(
        ...
        *,
        ...
        input_backend
            Keyboard input backend for base tele-op. Use ``"qt"`` to control
            the robot from the GUI window, or ``"pynput"`` / ``"curses"`` to
            run the terminal-based keyboard_controller.
        """
        ...
        # the streams.  When a non-Qt backend is selected, the terminal-based
        # keyboard_controller handles tele-op in its own thread instead.
        self._bot = None
        if self._input_backend == "qt":
            try:
                from hanger_boot_sequence import hanger_boot_sequence  # type: ignore
...
# need the separate `_keyboard_thread`.
        ...
        if self._input_backend != "qt":
            self._threads.append(
                threading.Thread(
                    target=_run_keyboard_controller,
                    args=(self._stop_evt, iface, self._input_backend),
                    daemon=True,
                )
            )
...
def main() -> None:  # noqa: D401
    ...
    parser.add_argument(
        "--input",
        choices=("qt", "pynput", "curses"),
        default="qt",
        help="keyboard input backend for base tele-op (default: qt)",
    )
