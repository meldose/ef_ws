...
# This node publishes a JointState message to /joint_states with a bow motion.
class Bow(Node):
    def __init__(self):
        super().__init__("rviz_joint_bow")
...
# This function is called every 20ms by the timer. It computes the current time, creates a JointState message, and fills in the positions of the animated joints using sine waves.
    def tick(self):
        ...
        self.set_joint(msg, "waist_pitch_joint", waist_pitch)
        for side in ("left", "right"):
            self.set_joint(msg, f"{side}_hip_pitch_joint", hip_pitch)
            self.set_joint(msg, f"{side}_knee_joint", knee)

        self.pub.publish(msg)

# def main() initializes the ROS2 node, creates an instance of the Bow class, and starts spinning the node to process callbacks until interrupted.
def main():
    rclpy.init()
    node = Bow()
    try:
