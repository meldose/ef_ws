...
class Custom:
    ...
    def Init(self, skip_motion_switcher=False):
        self.msc = MotionSwitcherClient()
        self.msc.SetTimeout(5.0)
        self.msc.Init()

        if not skip_motion_switcher:
            status, result = self.msc.CheckMode()
            if result is None:
                raise RuntimeError(
                    "MotionSwitcherClient.CheckMode() returned None. "
                    "Is the motion switcher service reachable? "
                    "Verify network interface, domain id, and robot power state."
                )
            while result.get("name"):
                ...
                if result is None:
                    raise RuntimeError(
                        "MotionSwitcherClient.CheckMode() returned None while releasing mode. "
                        "Is the motion switcher service reachable?"
                    )
                time.sleep(1)
...
if __name__ == '__main__':
    ...
    # Work around CycloneDDS tracing/config logging crash by disabling tracing.
    ...
    custom = Custom()
    custom.Init(skip_motion_switcher=args.skip_motion_switcher)
    custom.Start()
