...
class JointSelection:
    ...
    def slider_min(self) -> float:
        ...
        mirrored_right_min = -float(self.right_max)
        return max(self.left_min, mirrored_right_min)
    ...
    def slider_max(self) -> float:
        ...
        mirrored_right_max = -float(self.right_min)
        return min(self.left_max, mirrored_right_max)


JOINT_SELECTIONS = [
    JointSelection(
        name=WAIST_JOINT_NAMES[joint],
        label=f"waist: {WAIST_JOINT_NAMES[joint]}",
        joint_type="waist",
        left_index=joint,
        right_index=None,
        left_min=-2.5,
    ...
    JointSelection(
        name=spec.name,
        label=f"arms: {spec.name}",
        joint_type="arm",
        left_index=spec.left_index,
        right_index=spec.right_index,
        left_min=spec.left_min,
        left_max=spec.left_max,
        right_min=spec.right_min,
        right_max=spec.right_max,
        right_sign=spec.right_sign,
    )
    for spec in JOINT_SPECS
]
JOINT_SELECTION_BY_NAME = {selection.name: selection for selection in JOINT_SELECTIONS}
...
class DualArmMirrorWindow(QWidget):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.selected_joint_name = JOINT_SELECTIONS[0].name
    ...
    def _build_ui(self) -> None:
        ...
        for selection in JOINT_SELECTIONS:
            self.joint_combo.addItem(selection.label, selection.name)
        self.joint_combo.currentIndexChanged.connect(self._on_joint_selected)
        controls.addRow("Joint", self.joint_combo)
        ...
        self.setLayout(root)
        self.setWindowTitle("Dual Arm Mirror Joint UI With Waist")
        self.resize(920, 560)
    ...
    def _seed_from_state_if_available(self) -> None:
        ...
        self._refresh_joint_selection_widgets()
        self.status_label.setText(f"Connected on {self.iface}; waist available in joint picker")

    def _selected_joint(self) -> JointSelection:
        return JOINT_SELECTION_BY_NAME[self.selected_joint_name]

    def _selected_slider_value(self) -> float:
        selection = self._selected_joint()
        return float(self.desired_targets[selection.left_index])

    def _set_selected_slider_value(self, value: float) -> None:
        selection = self._selected_joint()
        clamped = max(selection.slider_min, min(selection.slider_max, float(value)))
        self.desired_targets[selection.left_index] = clamped
        if selection.right_index is not None:
            self.desired_targets[selection.right_index] = clamped * selection.right_sign
    ...
    def _refresh_joint_selection_widgets(self) -> None:
        ...
        self.slider_min_label.setText(f"{selection.slider_min: .3f}")
        self.slider_max_label.setText(f"{selection.slider_max: .3f}")
        if selection.right_index is None:
            self.mapping_label.setText(f"Waist {selection.name} = x")
        else:
            sign_text = "+" if selection.right_sign > 0.0 else "-"
            self.mapping_label.setText(
                f"Left {selection.name} = x, Right {selection.name} = {sign_text}x"
            )
    ...
    def _refresh_labels(self) -> None:
        ...
        self.current_label.setText(
            ...
            f"Target L/R: {target: .3f} / {right_target: .3f} rad"
        )
    ...
    def _load_saved_poses(self) -> None:
        ...
        self.sequence_steps = [
            ...
            if 0 <= int(step.get("pose_index", -1)) < len(self.saved_poses)
        ]
        self._refresh_pose_list()
    ...
    def _apply_pose_to_desired_targets(self, pose: dict[str, Any], *, include_waist: bool = True) -> None:
    ...
    def _remove_selected_poses(self) -> None:
        ...
        self.sequence_steps = new_sequence_steps
    ...
    def _refresh_sequence_list(self) -> None:
        ...
        for order, step in enumerate(self.sequence_steps, start=1):
            pose_index = int(step.get("pose_index", -1))
    ...
    def _add_selected_poses_to_sequence(self) -> None:
        ...
        if not rows:
            QMessageBox.warning(self, "No Pose", "Select one or more saved poses first.")
            return
        include_waist = self.sequence_include_waist_box.isChecked()
        self.sequence_steps.extend(
            {"pose_index": row, "include_waist": include_waist}
            for row in rows
        )
        self._refresh_sequence_list()
        waist_text = "with waist" if include_waist else "without waist"
        self.status_label.setText(f"Added {len(rows)} pose(s) to the sequence {waist_text}")
    ...
    def _tick(self) -> None:
        ...
        self.status_label.setText(
            f"Publishing {self.rate_hz: .1f} Hz, ramp limit {self.max_speed_rad_s: .2f} rad/s, waist available"
        )
